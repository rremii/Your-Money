...
export const Account: FC<props> = ({ color, icon, name, balance, id, currency }) => {
  ...
  const OnClick = () => {
    ...
    dispatch(setEditAccount({
      icon, name, color, currency, id, balance
    }))
    dispatch(setEditAccountMenuType("edit"))
  }

  return <AccountTemplate OnClick={OnClick} key={name} balance={balance} name={name} iconNode={
