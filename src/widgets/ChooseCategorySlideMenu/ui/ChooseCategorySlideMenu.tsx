...
import { setAccount, setChooseCategorySlideMenu } from "@entities/CurTransaction/model/CurTransactionSlice.ts"
...
export const ChooseCategorySlideMenu = () => {
  ...
  const isMenuOpen = useTypedSelector(state => state.CurTransaction.isChooseCategorySlideMenu)
  const activeType = useTypedSelector(state => state.CurTransaction.type)
  const curAccId = useTypedSelector(state => state.CurAccount.id)
  const chosenAccount = useTypedSelector(state => state.CurTransaction.account)
