...
export class TransactionService {
  ...
  async createTransactionTransaction(
    ...
    try {
      ...
      console.error(err)
      throw new BadRequestException(ApiError.TRANSACTION_NOT_CREATED);
    } finally {
  ...
  async editTransaction(editTransactionDto: EditTransactionDto) {
    ...
    try {
      ...
      await queryRunner.rollbackTransaction();
      console.error(err)
      throw new BadRequestException(ApiError.TRANSACTION_NOT_EDITED);
    } finally {
      await queryRunner.release();
    }
  }
  ...
  async deleteTransactionByIdTransaction({ id }: DeleteTransactionsDto) {
    ...
    const manager = queryRunner.manager;
    try {
      const transaction = await this.deleteTransactionById({ id, manager });
      ...
      await queryRunner.rollbackTransaction();
      console.error(err)
      throw new BadRequestException(ApiError.TRANSACTION_NOT_DELETED);
    } finally {
      await queryRunner.release();
    }
  }
